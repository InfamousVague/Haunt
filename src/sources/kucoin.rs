...
impl KuCoinClient {
    ...
    pub async fn start_polling(&self) {
        ...
        loop {
            if let Err(e) = self.fetch_prices().await {
                ...
                self.price_cache.report_source_error(PriceSource::KuCoin, &e.to_string());
            }
    ...
    async fn fetch_prices(&self) -> anyhow::Result<()> {
        ...
        if !response.status().is_success() {
            ...
            warn!("KuCoin API returned {}: {}", status, &text[..text.len().min(200)]);
        ...
        for ticker in tickers {
            if let Some(symbol) = pair_to_symbol.get(ticker.symbol.as_str()) {
                let price: f64 = ticker.last
                    .as_ref()
                    .and_then(|p| p.parse().ok())
                    .unwrap_or(0.0);

                let volume_24h: f64 = ticker.vol_value
                    .as_ref()
